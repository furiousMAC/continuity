...
static gint hf_btcommon_apple_airpods_utp = -1;
...
//12 Handoff
static gint hf_btcommon_apple_copy = -1;
static gint hf_btcommon_apple_seq = -1;
static gint hf_btcommon_apple_authtag= -1;
static gint hf_btcommon_apple_12_data = -1;
//13 Tethering Target (WiFi Settings Page)
static gint hf_btcommon_apple_icloud = -1;
...
static gint hf_btcommon_apple_tethsrc_3gpp = -1;
static gint hf_btcommon_apple_tethsrc_cellbars = -1;
//15 Nearby Action (WiFi Join)
...
static gint hf_btcommon_apple_nearbyaction_setup_model = -1;
static gint hf_btcommon_apple_nearbyaction_setup_protover = -1;
...
//16 Nearby Info
static gint hf_btcommon_apple_primary_device = -1;
static gint hf_btcommon_apple_hotspot_status = -1;
static gint hf_btcommon_apple_airdrop_status = -1;
static gint hf_btcommon_apple_action_code = -1;
static gint hf_btcommon_apple_os = -1;
static gint hf_btcommon_apple_wifi_status = -1;
...
static const value_string apple_vals[] = {
    ...
    { 11, "Magic Switch" },
    { 12, "Handoff" },
    { 13, "Tethering Target (WiFi Settings)" },
    { 14, "Tethering Source (Instant Hotspot)" },
    { 15, "Nearby Action (WiFi Join Network)" },
    { 16, "Nearby Info" },
    { 18, "Type 18 - Unknown" }, //Has same length as Type 7 (airpods)
    { 0, NULL }
};

static const value_string apple_copy_vals[] = {
    { 0x8, "Yes" },
    { 0x0, "No" },
    { 0, NULL }
};
static const value_string apple_primary_vals[] = {
    { 0x1, "Yes" },
    { 0x0, "No" },
    { 0, NULL }
};
static const value_string apple_hotspot_status_vals[] = {
    { 0x1, "Hotspot On" },
    { 0x0, "Hotspot Off" },
    { 0, NULL }
};
static const value_string apple_airdrop_status_vals[] = {
    { 0x1, "AirDrop On" },
    { 0x0, "AirDrop Off" },
    { 0, NULL }
};
...
static const value_string nearby_3gpp_vals[] = {
    { 0x0, "4G (GSM)" },
    { 0x1, "1xRTT" },
    { 0x2, "GPRS" },
    { 0x3, "EDGE" },
...
static const value_string nearby_os_vals[] = {
    { 0x00, "iOS 10.x" },
    { 0x0c, "iOS 12.x" },
    { 0x10, "iOS 11.x" },
    { 0x18, "iOS 12.x" },
...
static const value_string nearby_wifi_vals[] = {
    { 0x0c, "WiFi Join/Association" },
    { 0x18, "WiFi Off" },
    { 0x1c, "WiFi On" },
    { 0x1a, "WiFi Off" }, 
    { 0x1d, "WiFi Join/Association" }, //Not 100% sure about this 
    { 0x1e, "WiFi On" },
    { 0x98, "N/A (Seen w/ Action Code 1" },
    { 0, NULL }
};
...
static const value_string ios_setup_model_vals[] = {
    { 0x2000, "iPhone 5,6,7,8" },
    { 0x2100, "iPhone X" },
    { 0x2200, "iPhone SE" },
    { 0, NULL }
};

static const value_string charging_vals[] = {
    { 0x00, "No"}, 
    { 0x01, "Yes"}, 
    { 0, NULL}
};

static const value_string airpods_utp_vals[] = {
...
dissect_eir_ad_data(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, bluetooth_eir_ad_data_t *bluetooth_eir_ad_data)
{
    ...
    while (offset < data_size) {
        ...
        case 0x01: /* Flags */
            /* furiousmac */
            //Changed to ret_uint for macOS detection.
            proto_tree_add_item_ret_uint(entry_tree, hf_btcommon_eir_ad_flags, tvb, offset, 1, ENC_NA, &apple_os_flag);
            /* furiousmac */
        ...
        case 0x0A: /* Tx Power Level */
            /* furiousmac */
	    iOS_13_flag = 1; //iOS13 uses Tx Power Level; other iOS versions do not.
            /* furiousmac */
        ...
        case 0xFF: /* Manufacturer Specific */ {
            ...
            if (company_id == 0x004C && length > 1) { /* APPLE */
                ...
                guint32     os_set = 0;
                const gchar *nearby_os;
                proto_tree  *manuf_tree, *tlv_tree;
                proto_item  *tlv_item;                
                ...
                while(tvb_reported_length_remaining(tvb, offset) != 0){
                    ...
                    offset += 2;
		    if(apple_os_flag == 0x06 && os_set == 0){
			proto_tree_add_string(tlv_tree, hf_btcommon_apple_os, tvb, offset, 1, "macOS");
                        os_set = 1;
		    }
		    else if(iOS_13_flag == 1 && os_set == 0){
			proto_tree_add_string(tlv_tree, hf_btcommon_apple_os, tvb, offset, 1, "iOS 13.x");
                        os_set = 1;
		    } 
                    switch(a_type){
                        case 1:
                        ...
                        case 3:   /*AirPrint*/
                            ...
                            break;
                        case 5:   /*AirDrop*/
                            proto_tree_add_item(tlv_tree, hf_btcommon_apple_airdrop_prefix, tvb, offset, 9, ENC_NA);
                        ...
                        case 7:   /*AirPods*/
                            ...
                            proto_tree_add_item(tlv_tree, hf_btcommon_apple_airpods_utp, tvb, offset + 3, 1, ENC_NA);
                            proto_tree_add_item(tlv_tree, hf_btcommon_apple_airpods_leftbattery, tvb, offset + 4 , 1, ENC_NA);
                            proto_tree_add_item(tlv_tree, hf_btcommon_apple_airpods_rightbattery, tvb, offset + 4 , 1, ENC_NA);
                            proto_tree_add_item(tlv_tree, hf_btcommon_apple_airpods_casecharging, tvb, offset + 5 , 1, ENC_NA);
                            proto_tree_add_item(tlv_tree, hf_btcommon_apple_airpods_rightcharging, tvb, offset + 5 , 1, ENC_NA);
                            proto_tree_add_item(tlv_tree, hf_btcommon_apple_airpods_leftcharging, tvb, offset + 5 , 1, ENC_NA);
                            proto_tree_add_item(tlv_tree, hf_btcommon_apple_airpods_casebattery, tvb, offset + 5 , 1, ENC_NA);
                        ...
                        case 11:   /*Magic Switch*/
                            ...
                            break;
                        case 12:   /*Handoff*/
                            proto_tree_add_item(tlv_tree, hf_btcommon_apple_copy, tvb, offset, 1, ENC_BIG_ENDIAN);
                            proto_tree_add_item(tlv_tree, hf_btcommon_apple_seq, tvb, offset + 1, 2, ENC_LITTLE_ENDIAN);
                            proto_tree_add_item(tlv_tree, hf_btcommon_apple_authtag, tvb, offset + 3, 1, ENC_NA);
                            proto_tree_add_item(tlv_tree, hf_btcommon_apple_12_data, tvb, offset + 4, a_length - 4, ENC_NA);
                            offset += a_length;
                            break;
                        case 13:   /*Tethering Target (WiFi Settings Page)*/
                            proto_tree_add_item(tlv_tree, hf_btcommon_apple_icloud, tvb, offset, a_length, ENC_NA);
                            offset += a_length;
                            break;
                        case 14:   /*Tethering Source (Instant Hotspot)*/
                            ...
                            proto_tree_add_item(tlv_tree, hf_btcommon_apple_tethsrc_battery, tvb, offset + 2, 2, ENC_NA);
                            proto_tree_add_item(tlv_tree, hf_btcommon_apple_tethsrc_3gpp, tvb, offset + 4, 1, ENC_NA);
                            proto_tree_add_item(tlv_tree, hf_btcommon_apple_tethsrc_cellbars, tvb, offset + 5, 1, ENC_NA);
                            offset += a_length;
                            break;
                     	case 15:   /*Nearby Action (WiFi Join'ing Network)*/
                            if(a_length != 2){
				...
				proto_tree_add_item_ret_uint(tlv_tree, hf_btcommon_apple_nearbyaction_type, tvb, offset + 1, 1, ENC_NA, &nearbyaction_type_val);
			        proto_tree_add_item(tlv_tree, hf_btcommon_apple_nearbyaction_auth, tvb, offset + 2, 3, ENC_NA);
				switch(nearbyaction_type_val){
                                    case 8: /*WiFi Password*/
				        proto_tree_add_item(tlv_tree, hf_btcommon_apple_nearbyaction_wifijoin_appleID, tvb, offset + 5, 3, ENC_NA);
				        proto_tree_add_item(tlv_tree, hf_btcommon_apple_nearbyaction_wifijoin_phonenumber, tvb, offset + 8, 3, ENC_NA);
			                proto_tree_add_item(tlv_tree, hf_btcommon_apple_nearbyaction_wifijoin_email, tvb, offset + 11, 3, ENC_NA);
				        proto_tree_add_item(tlv_tree, hf_btcommon_apple_nearbyaction_wifijoin_ssid, tvb, offset + 14, 3, ENC_NA);
                                        break;
                                    case 9: /*iPhone Setup*/
				        proto_tree_add_item(tlv_tree, hf_btcommon_apple_nearbyaction_setup_model, tvb, offset + 5, 2, ENC_NA);
			                proto_tree_add_item(tlv_tree, hf_btcommon_apple_nearbyaction_setup_protover, tvb, offset + 7, 1, ENC_NA);
                                        break;
                                    default: 
                                        proto_tree_add_item(tlv_tree, hf_btcommon_apple_nearbyaction_data, tvb, offset + 5, a_length - 5, ENC_NA);
                                        break;
                                }
                            }
			    else{
                        ...
                        case 16: /*Nearby Info*/
                            proto_tree_add_item(tlv_tree, hf_btcommon_apple_primary_device, tvb, offset, 1, ENC_NA);
                            proto_tree_add_item(tlv_tree, hf_btcommon_apple_hotspot_status, tvb, offset, 1, ENC_NA); //Not 100% sure about this

                            proto_tree_add_item(tlv_tree, hf_btcommon_apple_airdrop_status, tvb, offset, 1, ENC_NA);
                            proto_tree_add_item_ret_uint(tlv_tree, hf_btcommon_apple_action_code, tvb, offset, 1, ENC_NA, &action_code_val);
                            offset += 1; 
                            a_length -= 1;
		            nearby_os_val = tvb_get_guint8(tvb, offset);
		            nearby_os = try_val_to_str(nearby_os_val, nearby_os_vals);
			    if(apple_os_flag != 0x06 && iOS_13_flag != 1){
			        if(action_code_val == 0x01){
                                    if(nearby_os_val == 0x1a || nearby_os_val == 0x1e){
                                        proto_tree_add_string(tlv_tree, hf_btcommon_apple_os, tvb, offset, 1, "iOS 13.x");
                                    }
			            else if(nearby_os_val == 0x08 || nearby_os_val == 0x18 || nearby_os_val == 0x1c){
				        proto_tree_add_string(tlv_tree, hf_btcommon_apple_os, tvb, offset, 1, "iOS 12.x");
				    }	
				    else if(nearby_os_val == 0x00 || nearby_os_val == 0x10){
				        proto_tree_add_string(tlv_tree, hf_btcommon_apple_os, tvb, offset, 1, "iOS 11.x");
				    }
				}
				else{
				    proto_tree_add_string(tlv_tree, hf_btcommon_apple_os, tvb, offset, 1, nearby_os ? nearby_os : "unk");
				}
		            }
			    switch(nearby_os_val){
		                /* 10.x */
			        case 0x00:
			        case 0x08:
			        ...
			        case 0x18:
			        case 0x1c:
			            proto_tree_add_item(tlv_tree, hf_btcommon_apple_wifi_status, tvb, offset, 1, ENC_BIG_ENDIAN);
				    if(a_length > 1){
				        proto_tree_add_item(tlv_tree, hf_btcommon_apple_nearbyinfo_auth, tvb, offset + 1, a_length - 1, ENC_NA);
				    }
				    offset += a_length;
            ...
            //added else if below
            /* furiousmac */
            else if (company_id == 0x000F && tvb_get_guint8(tvb, offset) == 0) { /* 3DS profile Legacy Devices */
...
proto_register_btcommon(void)
{
    ...
    static hf_register_info hf[] = {
        ...
        { &hf_btcommon_apple_type,
          { "Type",                              "btcommon.apple_type",
            FT_UINT8, BASE_DEC, VALS(apple_vals), 0x0,
            NULL, HFILL }
        }, 
        { &hf_btcommon_apple_length,
          { "Length",                            "btcommon.apple_length",
            FT_UINT8, BASE_DEC, NULL, 0x0,
            NULL, HFILL }
        },
        //3 AirPrint
        { &hf_btcommon_apple_airprint_addrtype,
          { "AirPrint Address Type",                              "btcommon.apple_airprint_addrtype",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airprint_resourcepathtype,
          { "AirPrint Resource Path Type",                              "btcommon.apple_airprint_resourcepathtype",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airprint_securitytype,
          { "AirPrint Security Type",                              "btcommon.apple_airprint_securitytype",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airprint_qidport,
          { "AirPrint QID or TCP Port",                              "btcommon.apple_airprint_qidport",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airprint_ipaddr,
          { "IP Address",                              "btcommon.apple_airprint_ipaddr",
            FT_IPv6, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airprint_power,
          { "Measured Power",                              "btcommon.apple_airprint_power",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        //5 AirDrop
        { &hf_btcommon_apple_airdrop_prefix,
          { "AirDrop Prefix",                              "btcommon.apple_airdrop_prefix",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airdrop_appleid,
          { "Apple ID",                              "btcommon.apple_airdrop_appleid",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airdrop_phone,
          { "Phone Number",                              "btcommon.apple_airdrop_phone",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airdrop_email,
          { "Email",                              "btcommon.apple_airdrop_email",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airdrop_email2,
          { "Email 2",                              "btcommon.apple_airdrop_email2",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airdrop_suffix,
          { "AirDrop Suffix",                              "btcommon.apple_airdrop_suffix",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        //6 HomeKit
        { &hf_btcommon_apple_homekit_status,
          { "Status Flags",                              "btcommon.apple_homekit_status",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_homekit_deviceid,
          { "Device ID",                              "btcommon.apple_homekit_deviceid",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_homekit_category,
          { "Category",                              "btcommon.apple_homekit_category",
            FT_UINT16, BASE_HEX, homekit_category_vals, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_homekit_globalstatenum,
          { "Global State Number",                              "btcommon.apple_homekit_globalstatenum",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_homekit_confignum,
          { "Configuration Number",                              "btcommon.apple_homekit_confignum",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_homekit_compver,
          { "Compatible Version",                              "btcommon.apple_homekit_compver",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        //7 Proximity Pairing (AirPods)
        { &hf_btcommon_apple_airpods_prefix,
          { "AirPods Prefix",                              "btcommon.apple_airpods_prefix",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airpods_devicemodel,
          { "AirPods Device Model",                              "btcommon.apple_airpods_devicemodel",
            FT_UINT16, BASE_HEX, airpods_device_vals, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airpods_utp,
          { "AirPods UTP",                              "btcommon.apple_airpods_utp",
            FT_UINT8, BASE_HEX, airpods_utp_vals, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airpods_leftbattery,
          { "Left AirPod Battery",                              "btcommon.apple_airpods_leftbattery",
            FT_UINT8, BASE_DEC, NULL, 0x0F,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airpods_rightbattery,
          { "Right AirPod Battery",                              "btcommon.apple_airpods_rightbattery",
            FT_UINT8, BASE_DEC, NULL, 0xF0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airpods_casecharging,
          { "AirPods Case Charging",                              "btcommon.apple_airpods_casecharging",
            FT_UINT8, BASE_HEX, charging_vals, 0x40,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airpods_rightcharging,
          { "Right AirPod Charging",                              "btcommon.apple_airpods_rightcharging",
            FT_UINT8, BASE_HEX, charging_vals, 0x20,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airpods_leftcharging,
          { "Left AirPod Charging",                              "btcommon.apple_airpods_leftcharging",
            FT_UINT8, BASE_HEX, charging_vals, 0x10,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airpods_casebattery,
          { "AirPod Case Battery",                              "btcommon.apple_airpods_casebattery",
            FT_UINT8, BASE_DEC, NULL, 0x0F,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airpods_opencount,
          { "AirPods Open Count",                              "btcommon.apple_airpods_opencount",
            FT_UINT8, BASE_DEC, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airpods_devicecolor,
          { "AirPods Device Color",                              "btcommon.apple_airpods_devicecolor",
            FT_UINT8, BASE_HEX, airpods_color_vals, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airpods_suffix,
          { "AirPods Suffix",                              "btcommon.apple_airpods_suffix",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airpods_encdata,
          { "AirPods Encrypted Data",                              "btcommon.apple_airpods_encdata",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        //8 "Hey Siri"
        { &hf_btcommon_apple_siri_perphash,
          { "Perceptual Hash",                              "btcommon.apple_siri_perphash",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_siri_snr,
          { "Signal-to-Noise Ratio",                              "btcommon.apple_siri_snr",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_siri_confidence,
          { "Confidence Level",                              "btcommon.apple_siri_confidence",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_siri_deviceclass,
          { "Device Class",                              "btcommon.apple_siri_deviceclass",
            FT_UINT16, BASE_HEX, siri_device_vals, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_siri_randbyte,
          { "Random Byte",                              "btcommon.apple_siri_randbyte",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        //9 AirPlay
        { &hf_btcommon_apple_airplay_flags,
          { "AirPlay Flags",                              "btcommon.apple_airplay_flags",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airplay_seed,
          { "AirPlay Seed",                              "btcommon.apple_airplay_seed",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airplay_ip,
          { "AirPlay IP",                              "btcommon.apple_airplay_ip",
            FT_IPv4, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        //11 Magic Switch
        { &hf_btcommon_apple_magicswitch_data,
          { "Data",                              "btcommon.apple_magicswitch_data",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_magicswitch_confidence,
          { "Confidence on Wrist",                              "btcommon.apple_magicswitch_confidence",
            FT_UINT8, BASE_HEX, wrist_confidence_vals, 0x0,
            NULL, HFILL }
        },
        //12 Handoff
        { &hf_btcommon_apple_copy,
          { "Copy/Cut Performed",                "btcommon.apple_copy",
            FT_UINT8, BASE_HEX, apple_copy_vals, 0x0f,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_seq,
          { "Sequence Number (IV)",                   "btcommon.apple_seq",
            FT_UINT16, BASE_DEC, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_authtag,
          { "AES-GCM Auth Tag",                   "btcommon.apple_authtag",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_12_data,
          { "Encrypted Handoff Data",                "btcommon.apple_12_data",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        //13 Tethering Target (WiFi Settings Page)
        { &hf_btcommon_apple_icloud,
          { "iCloud ID",                         "btcommon.apple_icloud",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        //14 Tethering Source (Instant Hotspot)
        { &hf_btcommon_apple_tethsrc_version,
          { "Version",                  "btcommon.apple_tethsrc_version",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_tethsrc_flags,
          { "Flags",                  "btcommon.apple_tethsrc_flags",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_tethsrc_battery,
          { "Battery Life (%)",                  "btcommon.apple_tethsrc_battery",
            FT_UINT8, BASE_DEC, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_tethsrc_3gpp,
          { "Cellular Connection Type",          "btcommon.apple_tethsrc_3gpp",
            FT_UINT8, BASE_DEC, VALS(nearby_3gpp_vals), 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_tethsrc_cellbars,
          { "Cell Service Quality (Bars)",       "btcommon.apple_tethsrc_cellbars",
            FT_UINT8, BASE_DEC, NULL, 0x0,
            NULL, HFILL }
        },
        //15 Nearby Action (WiFi Join)
        { &hf_btcommon_apple_nearbyaction_flags,
          { "Flags",                              "btcommon.apple_nearbyaction_flags",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_nearbyaction_type,
          { "Type",                              "btcommon.apple_nearbyaction_type",
            FT_UINT8, BASE_HEX, VALS(nearbyaction_type_vals), 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_nearbyaction_auth,
          { "Auth Tag",                "btcommon.apple_nearbyaction_auth",
            FT_UINT24, BASE_HEX, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_nearbyaction_wifijoin_ssid,
          { "SSID",                              "btcommon.apple_nearbyaction_wifijoin_ssid",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_nearbyaction_wifijoin_appleID,
          { "Apple ID",                              "btcommon.apple_nearbyaction_wifijoin_appleID",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_nearbyaction_wifijoin_phonenumber,
          { "Phone Number",                              "btcommon.apple_nearbyaction_wifijoin_phonenumber",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_nearbyaction_wifijoin_email,
          { "Email",                              "btcommon.apple_nearbyaction_wifijoin_email",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_nearbyaction_setup_model,
          { "iPhone Model",          "btcommon.apple_nearbyaction_setup_model",
            FT_UINT16, BASE_HEX, VALS(ios_setup_model_vals), 0x0,
            NULL, HFILL }
        },
	{ &hf_btcommon_apple_nearbyaction_setup_protover,
          { "Protocol Version",                  "btcommon.apple_nearbyaction_setup_protover",
            FT_UINT8, BASE_DEC, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_nearbyaction_data,
          { "Unk Data",      "btcommon.apple_nearbyaction_data",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
	//16 Nearby Info
        { &hf_btcommon_apple_primary_device,
          { "Primary Device",                       "btcommon.apple_primary",
            FT_UINT8, BASE_DEC, VALS(apple_primary_vals), 0x10,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_hotspot_status,
          { "Hotspot Status",                       "btcommon.apple_hotspot_status",
            FT_UINT8, BASE_DEC, VALS(apple_hotspot_status_vals), 0x20,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_airdrop_status,
          { "AirDrop Status",                       "btcommon.apple_airdrop_status",
            FT_UINT8, BASE_DEC, VALS(apple_airdrop_status_vals), 0x40,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_action_code,
          { "Action Code",                       "btcommon.apple_action",
            FT_UINT8, BASE_DEC, VALS(action_vals), 0x0F,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_os,
          { "iOS Version",                       "btcommon.apple_os",
            FT_STRING, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btcommon_apple_wifi_status,
          { "WiFi Status",                       "btcommon.apple_wifi",
            FT_UINT8, BASE_HEX, nearby_wifi_vals, 0x0,
            NULL, HFILL }
        },
	{ &hf_btcommon_apple_nearbyinfo_auth,
          { "Auth Tag",                 "btcommon.apple_nearbyinfo_auth",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        //unks
        { &hf_btcommon_apple_data,
          { "unk Data",                          "btcommon.apple_data",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
